...
function Kanganboard() {
  ...
  const orderTickets = (tickets, criteria) => {
    switch (criteria) {
      case "Priority":
        // Define a mapping for priority levels
        const priorityOrder = { Urgent: 4, High: 3, Medium: 2, Low: 1, "No Priority": 0 };
        
        // Ensure sorting works for both grouped and ungrouped tickets
        return [...tickets].sort((a, b) => {
          const priorityA = priorityOrder[priorityMapping[a.priority]] || 0; // Map numerical priority to name
          const priorityB = priorityOrder[priorityMapping[b.priority]] || 0;
    ...
    <div className="kanban">
      <div className="dropdown-wrapper" ref={dropdownRef}>
        <button onClick={() => setDropdownOpen(!dropdownOpen)} className="display-btn">
          ...
          <span className="disp">Display</span>
              ...
              {orderedTickets.map((ticket) => {
                ...
                return (
                  <Card
                    ...
                    grouping={grouping} // Pass the current grouping
                  />
                );
              })}
            </div>
